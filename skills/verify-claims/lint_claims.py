...
def test_docstring(repo_root, test_dir, ref):
    """Docstring of the `path::symbol` named in a `test` evidence line — or None if
    unresolvable (file outside the checkout, unparseable, or symbol absent). None means
    'no parity opinion', so lint stays green when Claim tests live outside this working tree.
    """
    m = re.search(r"`([^`]+?)::([A-Za-z_]\w*)`", ref)
    if not m:
        return None
    fpath = Path(repo_root) / (test_dir or "") / m.group(1)
    if not fpath.exists():
        return None
    try:
...
def main():
    ...
    for e in real:
        ...
        if e.file_key == "verified":
            ...
            # PARITY: a Claim test's docstring must restate the claim — it must carry the Claim ID.
            ...
            for kind, text in items:
                ...
                if doc is not None and e.id not in doc:
                    errors.append(("PARITY_MISMATCH", fname, e.lineno,
                                   f"{e.id}: Claim test docstring does not carry the Claim ID — the "
                                   f"docstring must restate the claim (the ledger is the source of truth)"))
